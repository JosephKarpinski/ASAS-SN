"""
hetdex_lae_lum_stack_v2.py
===========================
v2 improvements over hetdex_lae_lum_stack.py:
  1. MIN_N_STACK threshold: cells with N < MIN_N_STACK are treated as
     unreliable and shown as hatched grey panels with an explicit
     N= warning — preventing spurious EW/β values from biasing summary plots.
  2. AGN split in brightest logL bin: the [43.5,44.5) bin is stacked
     separately for LAE-classified and AGN-classified sources so the
     AGN-driven EW upturn is separated from genuine LAE behaviour.
  3. Summary EW(L) panels now overlay LAE-only and AGN tracks side-by-side.
  4. N annotations in grid cells colour-coded green/yellow/red by reliability.
...
SAVE_PATH  = "hetdex_lae_lum_stack_v2.png"
CSV_PATH   = "hetdex_lae_lum_stack_v2.csv"
...
# Minimum reliable stack size
# Cells with N_good < MIN_N_STACK are hatched in the figure
# and excluded from the summary EW(L) and beta(L) panels.
MIN_N_STACK = 50
...
# Resolve AGN flag column (present in SC2 detinfo; may not be in spec INFO table)
try:
    AGN_COL  = getcol(info, "agn_flag", "agnflag")
    agn_arr  = np.array(info[AGN_COL], dtype=int)
    HAS_AGN_FLAG = True
    print(f"  agn_flag column found: '{AGN_COL}'")
except KeyError:
    # Fall back to source_type == 'agn' if agn_flag absent
    agn_arr      = (np.array([s.strip().lower() for s in info[STYPE_COL]],
                              dtype=str) == "agn").astype(int)
    HAS_AGN_FLAG = False
    print("  agn_flag absent — using source_type='agn' as AGN indicator")
...
# grid[iz][il]         = combined LAE+AGN stack (for spectral panels)
# grid_lae[iz][il]     = LAE-only stack        (for EW/β summary panels)
# grid_agn[iz][il]     = AGN-only stack        (bright bin only)
# grid_n_raw[iz][il]   = raw count before downsampling (for reliability flag)
...
for iz, (z_lo, z_hi) in enumerate(Z_SLICES):
    ...
    for il in range(N_L):
        ...
        # Combined stack (all sources)
        st = build_stack(idx_all, label=label) if n_raw >= 3 else None

        # LAE-only sub-stack: exclude sources flagged as AGN
        lae_only  = cell_mask & (agn_arr == 0)
        idx_lae   = np.where(lae_only)[0]
        st_lae    = build_stack(idx_lae) if len(idx_lae) >= 3 else None

        # AGN sub-stack: only for the brightest bin to save compute
        st_agn = None
        if il == BRIGHT_BIN_IDX:
            agn_only = cell_mask & (agn_arr == 1)
            idx_agn  = np.where(agn_only)[0]
            st_agn   = build_stack(idx_agn) if len(idx_agn) >= 3 else None
            print(f"  [LAE={len(idx_lae):,} AGN={len(idx_agn):,}]", end="")
        else:
            print(f"  [LAE={len(idx_lae):,}]", end="")
        ...
        row.append(st); row_lae.append(st_lae)
        row_agn.append(st_agn)

    grid.append(row); grid_lae.append(row_lae)
    grid_agn.append(row_agn); grid_n_raw.append(row_n)
...
for iz, (z_lo, z_hi) in enumerate(Z_SLICES):
    for il in range(N_L):
        ...
        n_good_c = st["n_good"] if st is not None else 0
        reliable = n_good_c >= MIN_N_STACK

        if st is None or n_good_c < 3:
            # Empty cell — red border + N label
            for sp in ax.spines.values():
                sp.set_edgecolor("#f78166"); sp.set_linewidth(1.5)
            ax.text(0.5, 0.58, "insufficient\ndata",
                    ha="center", va="center",
                    transform=ax.transAxes, color=MUTED, fontsize=8)
            ax.text(0.5, 0.22, "N={}".format(n_raw),
                    ha="center", va="center", transform=ax.transAxes,
                    color="#f78166", fontsize=9, fontweight="bold")
        else:
            # Orange border for marginal cells
            if not reliable:
                for sp in ax.spines.values():
                    sp.set_edgecolor("#ffa657"); sp.set_linewidth(1.2)
            fade  = 1.0 if reliable else 0.40
            nc    = st["n_contrib"]
            MIN_C = max(3, n_good_c // 20)
            good  = nc >= MIN_C
            flux_sm = np.convolve(
            ...
            ax.fill_between(
                ...
                (st["flux"] - st["err"])[good],
                (st["flux"] + st["err"])[good],
                color=color, alpha=0.18 * fade, zorder=2)
            # Raw stack
            ax.plot(rest_wave[good], st["flux"][good],
                    color=color, lw=0.5, alpha=0.40 * fade, zorder=3)
            # Smoothed
            ax.plot(rest_wave[good], flux_sm[good],
                    color=TEXT, lw=(1.3 if reliable else 0.8),
                    alpha=0.90 * fade, zorder=4)
        ...
        # N badge — colour-coded by reliability
        n_col = ("#3fb950" if reliable else
                 "#ffa657" if n_good_c >= 10 else "#f78166")
        n_lbl = "N={:,}".format(n_good_c) if st else "N={}".format(n_raw)
        ax.text(0.50, 0.03, n_lbl, transform=ax.transAxes,
                color=n_col, fontsize=6.5, ha="center", va="bottom",
                fontweight="bold" if not reliable else "normal")
        if not reliable and st is not None:
            ax.text(0.50, 0.10, "LOW N", transform=ax.transAxes,
                    color="#ffa657", fontsize=6.0, ha="center",
...
for iz, (z_lo, z_hi) in enumerate(Z_SLICES):
    ...
    def collect_ew(gr, min_n=MIN_N_STACK):
        lms, ews_, errs_ = [], [], []
        for il in range(N_L):
            st_ = gr[iz][il]
            if st_ is None or st_["n_good"] < min_n:
                continue
            lf_ = st_["lya_fit"]
            if lf_ is None:
                continue
            lms.append(st_["logL_med"])
            ews_.append(lf_["ew_rest"])
            errs_.append(lf_["ew_rest"] * lf_["amp_err"]
                         / max(lf_["amp"], 1e-10))
        return np.array(lms), np.array(ews_), np.array(errs_)

    lm, ew, ewe = collect_ew(grid_lae)
    ...
    ax.errorbar(lm, ew, yerr=ewe,
                fmt="o", color="#58a6ff", ms=7, lw=1.5,
                capsize=3, elinewidth=1.2, zorder=5,
                label="LAE only")

    # LAE power-law fit
    try:
        coeffs_l = np.polyfit(lm, np.log10(np.clip(ew, 1, None)), 1)
        xl = np.linspace(lm.min() - 0.1, lm.max() + 0.1, 100)
        ax.plot(xl, 10**np.polyval(coeffs_l, xl),
                "--", color="#58a6ff", lw=1.3, alpha=0.75,
                label="slope={:.2f}".format(coeffs_l[0]))
    except Exception:
        pass

    # AGN overlay on brightest bin
    st_agn_cell = grid_agn[iz][BRIGHT_BIN_IDX]
    if st_agn_cell and st_agn_cell["n_good"] >= 10:
        lf_a = st_agn_cell["lya_fit"]
        if lf_a:
            ew_agn  = lf_a["ew_rest"]
            err_agn = ew_agn * lf_a["amp_err"] / max(lf_a["amp"], 1e-10)
            ax.errorbar([st_agn_cell["logL_med"]], [ew_agn],
                        yerr=[err_agn],
                        fmt="s", color="#f78166", ms=8, lw=1.5,
                        capsize=3, elinewidth=1.2, zorder=6,
                        label="AGN (N={})".format(st_agn_cell["n_good"]))

    # Combined (original grid) as faint reference
    lm_c, ew_c, ewe_c = collect_ew(grid, min_n=10)
    if len(lm_c) >= 2:
        ax.errorbar(lm_c, ew_c, yerr=ewe_c,
                    fmt="^", color="#ffa657", ms=4, lw=0.8,
                    capsize=2, elinewidth=0.8, alpha=0.45, zorder=4,
                    label="All (LAE+AGN)")
...
for iz, (z_lo, z_hi) in enumerate(Z_SLICES):
    ...
    def collect_beta(gr, min_n=MIN_N_STACK):
        lms_, bs_, bes_ = [], [], []
        for il in range(N_L):
            st_ = gr[iz][il]
            if st_ is None or st_["n_good"] < min_n:
                continue
            if not np.isfinite(st_["beta"]):
                continue
            lms_.append(st_["logL_med"])
            bs_.append(st_["beta"])
            bes_.append(st_["beta_err"])
        return np.array(lms_), np.array(bs_), np.array(bes_)

    lm_b, bv, be = collect_beta(grid_lae)
    ...
    # LAE-only β points
    ax.errorbar(lm_b, bv, yerr=be,
                fmt="s", color="#3fb950", ms=7, lw=1.5,
                capsize=3, elinewidth=1.2, zorder=5, label="LAE only")

    # Trend line
    try:
        coeffs_b = np.polyfit(lm_b, bv, 1)
        xl_b = np.linspace(lm_b.min() - 0.1, lm_b.max() + 0.1, 100)
        ax.plot(xl_b, np.polyval(coeffs_b, xl_b),
                "--", color="#3fb950", lw=1.3, alpha=0.75,
                label="slope={:.2f}".format(coeffs_b[0]))
    except Exception:
        pass

    # AGN β on brightest bin
    st_agn_b = grid_agn[iz][BRIGHT_BIN_IDX]
    if st_agn_b and st_agn_b["n_good"] >= 10 and np.isfinite(st_agn_b["beta"]):
        ax.errorbar([st_agn_b["logL_med"]], [st_agn_b["beta"]],
                    yerr=[st_agn_b["beta_err"]],
                    fmt="D", color="#f78166", ms=8, lw=1.5,
                    capsize=3, elinewidth=1.2, zorder=6,
                    label="AGN (N={})".format(st_agn_b["n_good"]))
...
print("=" * 88)
hfmt = "  {:>4}  {:>12}  {:>14}  {:>6}  {:>7}  {:>8}  {:>7}  {:>7}  {:>6}"
rfmt = "  {:>4}  {:>12}  {:>14}  {:>6}  {:>7.3f}  {:>8.1f}  {:>7.2f}  {:>7.2f}  {:>6}"
print(hfmt.format("type","z-slice","logL-bin","N","z_med",
                  "EW_rest","FWHM","beta","beta_e"))
print("  " + "-"*86)
for iz, (z_lo, z_hi) in enumerate(Z_SLICES):
    for il in range(N_L):
        for tag, gr in [("ALL", grid), ("LAE", grid_lae)]:
            st = gr[iz][il]
            if st is None:
                continue
            flag = "  *** LOW N ***" if st["n_good"] < MIN_N_STACK else ""
            lf   = st["lya_fit"]
            ew_s   = "{:.1f}".format(lf["ew_rest"]) if lf else "n/a"
            fwhm_s = "{:.2f}".format(lf["fwhm"])    if lf else "n/a"
            b_s    = "{:.2f}".format(st["beta"])     if np.isfinite(st["beta"]) else "n/a"
            be_s   = "{:.2f}".format(st["beta_err"]) if np.isfinite(st["beta_err"]) else "n/a"
            zbin_s = "[{},{})".format(z_lo, z_hi)
            lbin_s = "[{:.2f},{:.2f})".format(l_edges[il], l_edges[il+1])
            row_str = rfmt.format(
                tag, zbin_s, lbin_s, st["n_good"],
                st["z_med"],
        ...
        if il == BRIGHT_BIN_IDX:
            st_a = grid_agn[iz][il]
            if st_a is not None and st_a["n_good"] >= 3:
                lf_a   = st_a["lya_fit"]
                ew_a   = "{:.1f}".format(lf_a["ew_rest"]) if lf_a else "n/a"
                fwhm_a = "{:.2f}".format(lf_a["fwhm"])    if lf_a else "n/a"
                b_a    = "{:.2f}".format(st_a["beta"])     if np.isfinite(st_a["beta"]) else "n/a"
                be_a   = "{:.2f}".format(st_a["beta_err"]) if np.isfinite(st_a["beta_err"]) else "n/a"
                lbin_s = "[{:.2f},{:.2f})".format(l_edges[il], l_edges[il+1])
                row_str = rfmt.format(
                    "AGN", "[{},{})".format(z_lo,z_hi), lbin_s, st_a["n_good"],
                    st_a["z_med"],
                    float(ew_a) if ew_a != "n/a" else 0,
                    float(fwhm_a) if fwhm_a != "n/a" else 0,
                    float(b_a) if b_a != "n/a" else 0,
                    be_a,
                )
                print(row_str)
    print()
